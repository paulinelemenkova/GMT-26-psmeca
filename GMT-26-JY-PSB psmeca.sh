...
#grdcut GEBCO_2019.nc -R120/152/4/35 -Gpsb_relief.nc
ps=Geol_PSB_meca.ps
# GMT set up
...
#grdcut earth_relief_01m.grd -R120/152/4/35 -Gpsb_relief.nc
# Step-6. Make raster image
#gmt grdimage psb_relief.nc -Cetopo1.cpt -R120/152/4/35 -JM16c -P -I+a15+ne0.75 -Xc -K > $ps
# Add elemens of basemap: title, grids, rose, scale, time stamp
#gmt psbasemap -R -J \
    -B+t"Seismic settings of the PSB: focal mechanisms of the earthquakes" \
    -Bpxg8f2a4 -Bpyg6f3a3 -Bsxg4 -Bsyg3 \
    -Lx13.0c/-1.0c+c50+w500k+l"Behrman cylindrical projection. Scale (km)"+f \
    -UBL/-0.2c/-1.0c -O -K >> $ps
gmt psbasemap -R120/152/4/35 -JM16c \
    -B+t"Velocity ellipses in (N,E) convention and rotational wedges, PSB margins" \
    -Bpxg8f2a4 -Bpyg6f3a3 -Bsxg4 -Bsyg3 \
    -Lx13.0c/-1.0c+c50+w500k+l"Behrman cylindrical projection. Scale (km)"+f \
    -UBL/-0.2c/-1.0c -K > $ps
...
gmt psmeca -R CMT.txt -J -Sd0.4/10/u -Gred -L0.1p -O >> $ps
...
0.5 15.5 Dataset: Global CMT Catalog (Harvard CMT). Basemap: GEBCO bathymetric 15 arc sec grid
EOF
# Add GMT logo
gmt logo -Dx6.3/-0.8+o0.3c/-8.2c+w2c -O >> $ps
# Convert to image file using GhostScript (portrait orientation, 720 dpi)
gmt psconvert Geol_PSB_meca.ps -A0.5c -E720 -Tj -P -Z
